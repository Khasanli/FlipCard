//
//  FlipSecondViewController.swift
//  FlipYCard
//
...
class FlipSecondViewController: UIViewController {

    weak var frontHeightConstraint : NSLayoutConstraint?
    weak var backHeightConstraint : NSLayoutConstraint?
    ...
    func setupFlipView() {
        ...
        frontHeightConstraint = frontSideView.heightAnchor.constraint(equalToConstant: 170)
        frontHeightConstraint?.isActive = true

        backHeightConstraint = backSideView.heightAnchor.constraint(equalToConstant: 170)
        ...
        NSLayoutConstraint.activate([
            ...
            flipView.heightAnchor.constraint(equalToConstant: height),
...
extension FlipSecondViewController: BottomSheetDelegate {
    func didPanBottomSheet(percentage: CGFloat, duration: CGFloat) {
        ...
        if percentage > 0.5 {
            let adjustedPercentage = (percentage - 0.5) * 2
            let newHeight = height * adjustedPercentage
            frontSideView.layer.cornerRadius = 10 * adjustedPercentage
            frontHeightConstraint?.constant = newHeight
            backHeightConstraint?.constant = 0
        } else {
            let adjustedPercentage = (0.5 - percentage) * 2
            let newHeight = height * adjustedPercentage
            backHeightConstraint?.constant = newHeight
            backSideView.layer.cornerRadius = 10 * adjustedPercentage
            frontHeightConstraint?.constant = 0
        }
